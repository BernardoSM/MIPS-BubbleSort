...
.text
	...
	li $v0, 4
	syscall
	lw $a0, 0($s0) 	#o syscall sempre vai exibir o valor que está em $a0
	li $v0, 1
	syscall
	la $a0, barra
	li $v0, 4
	syscall
	lw $a0, 4($s0) 	#o syscall sempre vai exibir o valor que está em $a0
	li $v0, 1
	syscall
	la $a0, barra
	li $v0, 4
	syscall
	lw $a0, 8($s0) 	#o syscall sempre vai exibir o valor que está em $a0
	li $v0, 1
	syscall
	#FIM PRINTF DO VETOR
...
trocapos:	
	...
	addi $a0, $a0, 4	#$a0 += 4 // Endere?o = endereço + 4 (avança uma posição do vetor)
	addi $t2, $t2, 1	#$t2++
		j ini_loop
